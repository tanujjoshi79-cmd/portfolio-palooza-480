...
async function smartApi<T>(path: string, body: unknown, jwtToken?: string): Promise<T> {
  ...
  if (!response.ok || json.status === false) {
    throw new Error(json.message || json.errorcode || `Angel One API error (${response.status})`);
  }
...
async function login(): Promise<SessionData> {
  ...
  const password = requiredEnv("ANGEL_PASSWORD_OR_PIN");
  const totp = await generateTotp(requiredEnv("ANGEL_TOTP_SECRET"));
...
async function resolveScrip(jwtToken: string, symbol: string, exchange: string): Promise<ScripData> {
  const result = await smartApi<{ data: ScripData[] }>(
    "/rest/secure/angelbroking/order/v1/searchScrip",
    { exchange, searchscrip: symbol },
    jwtToken,
  );
  const matches = Array.isArray(result) ? result : (result as unknown as ScripData[]);
  const exact = matches.find(
    (item) => item.exchange === exchange && item.tradingsymbol.toUpperCase() === symbol.toUpperCase(),
  );
...
export async function placeLiveOrder(input: LiveOrderInput) {
  if (process.env.LIVE_TRADING_ENABLED !== "true") {
    throw new Error("Live trading is disabled. Set LIVE_TRADING_ENABLED=true on the server after testing.");
  }
  ...
  if (orderType === "LIMIT" && (!input.price || input.price <= 0)) {
  ...
  const order = await smartApi<{ orderid?: string }>(
