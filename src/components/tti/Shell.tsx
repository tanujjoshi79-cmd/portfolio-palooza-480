      ...
      <div className="relative">
        <header className="sticky top-0 z-30 border-b border-line bg-background/70 backdrop-blur-xl">
          <div className="flex items-center justify-between gap-4 px-5 py-3">
            <div className="flex min-w-0 items-center gap-8">
              ...
              <nav className="hidden items-center gap-5 font-mono text-[11px] uppercase tracking-[0.15em] text-muted-foreground md:flex">
                {nav.map((n) => <Link key={n.to} to={n.to} activeOptions={{ exact: n.to === "/" }} activeProps={{ className: "text-foreground" }} className="transition-colors hover:text-foreground/80">{n.label}</Link>)}
              </nav>
            </div>

            <div className="relative hidden min-w-0 flex-1 max-w-md md:block">
              <div className="flex items-center gap-2 rounded-lg border border-line bg-panel/60 px-3 py-2 focus-within:border-white/20">
                <span className="font-mono text-[11px] text-muted-foreground">⌕</span>
                <input
                  value={query}
                  onChange={(e) => { setQuery(e.target.value); setOpen(true); }}
                  onFocus={() => setOpen(true)}
                  onKeyDown={(e) => { if (e.key === "Escape") { setOpen(false); setQuery(""); } if (e.key === "Enter" && results[0]) goToStock(results[0].symbol); }}
                  placeholder="Search stocks, companies, sectors..."
                  aria-label="Search stocks, companies and sectors"
                  className="min-w-0 flex-1 bg-transparent font-mono text-[11px] outline-none placeholder:text-muted-foreground"
                />
                <kbd className="rounded border border-line px-1.5 py-0.5 font-mono text-[9px] text-muted-foreground">/</kbd>
              </div>
              {open && query.trim() && (
                <div className="absolute left-0 right-0 top-full mt-2 overflow-hidden rounded-xl border border-line bg-[#111218] shadow-2xl">
                  {results.length ? results.map((s) => (
                    <button key={s.symbol} type="button" onMouseDown={(e) => e.preventDefault()} onClick={() => goToStock(s.symbol)} className="flex w-full items-center justify-between border-b border-line px-3 py-3 text-left last:border-0 hover:bg-white/[0.05]">
                      <span><span className="block font-mono text-[12px] font-semibold">{s.symbol}</span><span className="text-[10px] text-muted-foreground">{s.name} · {s.sector}</span></span>
                      <span className={`font-mono text-[11px] ${s.changePct >= 0 ? "text-up" : "text-down"}`}>{s.changePct >= 0 ? "+" : ""}{s.changePct.toFixed(2)}%</span>
                    </button>
          ...
          <div className="flex overflow-x-auto border-t border-line bg-panel/40">
            {indices.map((i, idx) => <div key={i.name} className="flex items-center">{idx > 0 && <div className="h-4 w-px bg-line" />}<div className="flex items-center gap-2 whitespace-nowrap px-4 py-2"><span className="font-mono text-[11px] font-medium">{i.name}</span><span className="font-mono text-[11px]">{i.value}</span><span className={`font-mono text-[11px] ${i.changePct >= 0 ? "text-up" : "text-down"}`}>{i.changePct >= 0 ? "+" : ""}{i.changePct.toFixed(2)}%</span></div></div>)}
          </div>
        </header>
